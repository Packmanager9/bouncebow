...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    let circs = []
    for (let t = 0; t < 2560*1; t++) {
        let circ = new Circle(t*.5, 360, 2, `rgb(${(t / 2) - ((t / 100) * (t / 40))}, ${(255 - (t / 2.5)) + ((t / 100) * (t / 60))}, ${(t / 120) * (t / 100)})`, 0, (t + 1) / 10, 1, 1)
        circs.push(circ)
    }
    ...
    function main() {
        ...
        if (keysPressed['='] && recording == 1) {
            recording = 0
            video_recorder.stop()
            video_recorder.download('bounce.webm')
        }
    }
